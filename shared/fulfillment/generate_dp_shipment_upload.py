...
def validate_rows(
    ...
    if duplicate_tracking and consolidation_plan is None:
        raise ValueError(
            f"Duplicate tracking/waybill in {source_label} rows: "
            + json.dumps(duplicate_tracking, ensure_ascii=False)
        )
    if duplicate_tracking and consolidation_plan is not None:
        order_packages = order_consolidation.order_to_package(consolidation_plan)
        for tracking_number in duplicate_tracking:
            tracking_rows = [row for row in rows if row["Tracking Number"] == tracking_number]
            references = {row.get("Carrier Reference", "") for row in tracking_rows}
            if len(references) != 1:
                raise ValueError(
                    f"Tracking number {tracking_number} spans multiple carrier references: "
                    + json.dumps(sorted(references), ensure_ascii=False)
                )
            reference = next(iter(references))
            package = next(
                (
                    package
                    for package in consolidation_plan["packages"]
                    if package["carrier_reference"] == reference
                ),
                None,
            )
            actual_order_ids = {row["Order ID"] for row in tracking_rows}
            expected_group_order_ids = {
                line["order_id"] for line in package["order_lines"]
            } if package else set()
            if (
                not package
                or not package["merged"]
                or actual_order_ids != expected_group_order_ids
                or any(order_packages.get(order_id) is not package for order_id in actual_order_ids)
            ):
                raise ValueError(
                    f"Duplicate tracking number {tracking_number} is not authorized by one merged package."
                )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Consolidation JSON generated by step 1; expands one carrier parcel to all original DP orders.",
    )
    ...
    try:
        ...
        if args.sf_international_file:
            ...
            carrier = SF_INTERNATIONAL_CARRIER
            carrier_rows = read_sf_international_rows(args.sf_international_file)
            platform_key = "sf"
        else:
            source_label = "YunExpress"
            carrier = YUNEXPRESS_CARRIER
            carrier_rows = read_yunexpress_rows(args.yunexpress_xlsx, args.prefer_tracking)
            platform_key = "yunexpress"
        consolidation_plan = None
        if args.merge_map:
            if not args.dp_orders_csv:
                raise ValueError("Using a consolidation plan requires the original DP order CSV.")
            ...
            rows = order_consolidation.expand_carrier_rows(carrier_rows, consolidation_plan)
        else:
            rows = carrier_rows
        validate_rows(rows, expected_order_ids, source_label, carrier, consolidation_plan)
        ...
        expected_tracking_count = (
            int(consolidation_plan["package_count"])
        ...
        if (
            ...
            or validation["tracking_unique_count"] != expected_tracking_count
        ):
        ...
        report = {
            ...
            "tracking_map_path": str(tracking_map_path) if tracking_map_path else None,
            "consolidation": (
                {
    ...
    if tracking_map_path:
        print(f"TRACKING_MAP: {tracking_map_path}")
    return 0
