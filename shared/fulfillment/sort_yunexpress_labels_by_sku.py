...
def read_dp_orders(csv_path: Path, status: str = DEFAULT_DP_STATUS) -> dict[str, str]:
    ...
    if not order_to_skus:
        suffix = f" with Status={status!r}" if "Status" in fieldnames else ""
        raise ValueError(f"DP order CSV contains no usable orders{suffix}.")
...
def read_yunexpress_shipments(xlsx_path: Path) -> dict[str, Shipment]:
    ...
    try:
        ...
        for row in range(2, sheet.max_row + 1):
            ...
            shipment_status = clean(sheet.cell(row, status_col).value) if status_col else ""
