...
订单 CSV、合单计划、物流订单 Excel、云途面单 ZIP 和顺丰面单 PDF 只在对应账号、对应平台的批次目录中保存。第 2 步自动使用第 1 步 DP CSV 和合单计划校验订单集合；顺丰第 3 步自动使用第 2 步订单数据校验面单运单集合。失败上传会删除，上一份已验证批次不会被覆盖。第 3 步成功后清理对应批次目录。首次部署建议先使用脱敏文件测试，不要直接上传真实订单。

## 平台流程

- 云途：DP CSV -> 云途批量模板 -> DP 运单回填 -> 按 SKU 分拣供应商面单包。
- 顺丰国际：DP CSV -> 顺丰批量模板 -> DP 运单回填 -> 运单集合校验 -> 按寄方姓名拆分合并面单 PDF。
- 顺丰 DP 回填的承运商固定为 `SF INTERNATIONAL`。
- 第 2 步不要求再次上传 DP CSV，但必须先由同一账号、同一承运平台成功完成第 1 步。
- 第 1 步可合并收件人六项信息及 SKU 均一致的订单；第 2 步将一个运单号回填到组内全部 DP 订单；第 3 步单独生成合并件面单、供应商说明和 ZIP。
