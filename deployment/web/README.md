...
订单 CSV、物流订单 Excel、云途面单 ZIP 和顺丰面单 PDF 只在对应账号、对应平台的批次目录中保存。第 2 步自动使用第 1 步 DP CSV 校验订单集合；顺丰第 3 步自动使用第 2 步订单数据校验面单运单集合。失败上传会删除，上一份已验证批次不会被覆盖。第 3 步成功后清理对应批次目录。首次部署建议先使用脱敏文件测试，不要直接上传真实订单。
