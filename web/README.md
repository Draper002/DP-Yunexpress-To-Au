...
| 2 | 云途订单信息生成 DP 回填模板 | 顺丰订单 `.xls/.xlsx` 生成 DP 回填模板，承运商固定为 `SF INTERNATIONAL` |
...
本版本使用 SQLite 和磁盘保存账号、处理结果及校验记录。各账号的处理记录、工作目录和下载权限相互隔离；管理员可以下载全部结果。云途第 3 步成功后清理该账号的云途批次文件，顺丰第 3 步成功后清理顺丰批次文件。失败批次保留当前输入，便于排查，服务器需要配置定期清理策略。
...
2026-07-26 已使用真实结构文件分别完成云途和顺丰国际 Web 三步回归；测试运行目录位于本地 `artifacts/`，不进入 Git 或部署包。
